...
--  Version: 001
--  Created: 2026-03-24
--  Description: Multi-tenant schema for Job Automation SaaS
--               Tables: users, user_resumes, user_search_configs, jobs, runs
--               All tables have RLS policies filtering on user_id = auth.uid()
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_user_search_configs_user_id ON user_search_configs(user_id);
...
CREATE INDEX IF NOT EXISTS idx_jobs_first_seen ON jobs(user_id, first_seen);
CREATE INDEX IF NOT EXISTS idx_jobs_match_score ON jobs(user_id, match_score);
...
CREATE INDEX IF NOT EXISTS idx_runs_date ON runs(user_id, run_date);
