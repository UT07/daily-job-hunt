...
1. **ATS Score** — Applicant Tracking System perspective:
   ...
   - Formatting: Is the LaTeX clean and parseable (no tables/images that break ATS)?
...
Return ONLY valid JSON (no markdown, no code fences):
{
    ...
    "improvements": ["<specific LaTeX edit suggestion 1>", "<specific edit 2>", ...]
}"""


IMPROVE_SYSTEM_PROMPT = r"""You are an expert resume writer. You've been given a LaTeX resume that scored below 85 on one or more evaluations (ATS, Hiring Manager, Technical Recruiter).
...
2. Keep exact same LaTeX structure and commands.
3. Make surgical, targeted edits — don't rewrite everything.
4. Focus on the specific feedback provided.
5. Keep the resume to ONE PAGE.
6. Use the job listing's terminology where the candidate genuinely has the skill.

Return ONLY the complete, modified LaTeX source code. No explanations, no markdown fences.
...
def score_and_improve(
    ...
    Returns (final_tex, final_scores).
    """
